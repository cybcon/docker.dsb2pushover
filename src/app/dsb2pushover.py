...
if version.parse(dsbapi.__version__) < version.parse(dsbapi_min_version):
    print(
        "ERROR: DSBApi library version "
        + dsbapi.__version__
        + " found but require minimum version "
        + dsbapi_min_version
    )
    sys.exit(1)
...
def parse_dsb_data(dsb_entries, filter_class=None, filter_date=None):
    ...
    if filter_class and "class" in df.columns:
        ...
        df = df[df["class"].str.match(r"(" + str(filter_class) + ")") == True]  # noqa: E712
...
send_pushover_message(
    userkey=os.environ["PUSHOVER_USER_KEY"], apikey=os.environ["PUSHOVER_API_KEY"], title=SUBJECT, message=PAYLOAD
)
